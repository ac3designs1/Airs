...
const PANELS = [
  { title: 'User Management',    desc: 'Manage officer accounts, roles and access',    icon: Users,         to: '/users',             accent: '#0ea5e9', glow: 'rgba(14,165,233,0.15)'  },
...
export default function Admin() {
  const { auth } = useAuth();
  const isAdmin = ['admin', 'administrator', 'leadership', 'senior_command', 'supervisor'].includes(auth.user?.role ?? '');
  ...
  const kpis = [
    { label: 'Total Officers', value: stats?.total_officers ?? '—', color: '#0ea5e9', icon: Users },
      ...
      <div className="glass rounded-2xl p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={i.label} className="rounded-xl p-3" style={{ background: 'rgba(14,165,233,0.04)', border: '1px solid rgba(14,165,233,0.08)' }}>
