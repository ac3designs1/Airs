  ...
  return (
    <div className="min-h-screen" style={{ background: '#06060a' }}>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <div className="absolute w-[700px] h-[700px] rounded-full -top-60 left-1/2 -translate-x-1/2"
          style={{ background: 'radial-gradient(circle, rgba(168,85,247,0.14) 0%, rgba(124,58,237,0.06) 40%, transparent 70%)', filter: 'blur(40px)' }} />
        <div className="absolute w-[400px] h-[400px] rounded-full -bottom-40 -right-20"
          style={{ background: 'radial-gradient(circle, rgba(236,72,153,0.10) 0%, transparent 70%)', filter: 'blur(50px)' }} />
        <div className="absolute w-[300px] h-[300px] rounded-full bottom-1/3 -left-20"
          style={{ background: 'radial-gradient(circle, rgba(99,102,241,0.08) 0%, transparent 70%)', filter: 'blur(40px)' }} />
        <div className="absolute inset-0 opacity-[0.02]"
          style={{ backgroundImage: 'linear-gradient(rgba(168,85,247,0.8) 1px, transparent 1px), linear-gradient(90deg, rgba(168,85,247,0.8) 1px, transparent 1px)', backgroundSize: '40px 40px' }} />
      </div>

      <div className={`relative z-10 transition-all duration-700 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>

        {/* Nav */}
        <div className="flex items-center justify-between px-8 h-16" style={{ borderBottom: '1px solid rgba(168,85,247,0.10)' }}>
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg flex items-center justify-center"
              style={{ background: 'linear-gradient(135deg,#7c3aed,#a855f7)', boxShadow: '0 0 14px rgba(168,85,247,0.35)' }}>
              <Shield className="w-4 h-4 text-white" />
            </div>
            <div>
              <div className="text-white font-bold text-sm leading-none">NextAirs</div>
              <div className="text-[10px] font-mono" style={{ color: '#a855f7' }}>NEXT RP · MELBOURNE POLICE</div>
            </div>
          </div>
        ...
        <div className="text-center pt-14 pb-10 px-4">
          ...
          <h1 className="text-5xl md:text-6xl font-black text-white mb-4 leading-none tracking-tight" style={{ letterSpacing: '-0.03em' }}>
            Join <span style={{ background: 'linear-gradient(135deg,#e879f9,#a855f7)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', backgroundClip:'text' }}>Melbourne</span><br />Police Force
          </h1>
